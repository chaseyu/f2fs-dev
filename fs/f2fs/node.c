...
skip_partial:
	while (cont) {
		...
		if (offset[1] == 0 && get_nid(entry, offset[0], true)) {
			...
			BUG_ON(!f2fs_is_node_cache(entry));
				...
				enum iostat_type io_type, unsigned int *seq_id)
{
	...
	if (f2fs_sanity_check_node_footer(sbi, entry, nid,
...
int f2fs_write_node_caches(struct f2fs_sb_info *sbi, bool sync)
{
	...
	if (!sync && get_pages(sbi, F2FS_DIRTY_NODES) <
	...
	blk_start_plug(&plug);
	f2fs_writeback_node_caches(sbi, nr_to_write, sync, true, FS_NODE_IO);
	blk_finish_plug(&plug);
