...
static int make_empty_dir(struct inode *inode,
		struct inode *parent, struct f2fs_cached_block *entry)
{
	struct folio *dentry_folio;
	struct f2fs_dentry_block *dentry_blk;
	struct f2fs_dentry_ptr d;
	...
	if (f2fs_has_inline_dentry(inode))
		return f2fs_make_empty_inline_dir(inode, parent, entry);

	dentry_folio = f2fs_get_new_data_folio(inode, entry, 0, true);
	if (IS_ERR(dentry_folio))
		...
		const struct f2fs_filename *fname, struct f2fs_cached_block *dentry)
{
	...
	if (is_inode_flag_set(inode, FI_NEW_INODE)) {
		entry = f2fs_new_inode_cache(inode);
		if (IS_ERR(entry))
			return entry;
		...
		if (S_ISDIR(inode->i_mode)) {
			...
			err = make_empty_dir(inode, dir, entry);
			if (err) {
				f2fs_lock_cache(entry);
				goto put_error;
			}
		...
		err = f2fs_init_acl(inode, dir, entry, dentry);
		if (err)
			goto put_error;
		...
		if (IS_ENCRYPTED(inode)) {
			err = fscrypt_set_context(inode, entry);
			if (err)
				goto put_error;
		}
	} else {
		entry = f2fs_get_inode_cache(F2FS_I_SB(dir), inode->i_ino);
		if (IS_ERR(entry))
			return entry;
	}

	init_dent_inode(dir, inode, fname, entry);
			  ...
			  const struct f2fs_filename *fname)
{
	...
	make_dentry_ptr_inline(dir, &d, inline_data_addr(dir, entry));
