					...
					enum page_type type)
{
	...
	/* submit cached bio */
	f2fs_submit_merged_write_cache(sbi, entry, 0, type);
...
{
	enum page_type ptype = IS_META_CACHE(entry->cache) ? META : NODE;

	f2fs_cache_wait_writeback_cond(entry, ptype);
}
...
{
	folio_put(virt_to_folio(entry->data));
	kfree(entry);
}
...
static void f2fs_free_cache(struct f2fs_cached_block *entry)
{
	...
	BUG_ON(f2fs_cache_refcount(entry));
		...
		unsigned long index, bool nofail)
{
	...
	entry->data = folio_address(folio);
	entry->index = index;
...
static int f2fs_cache_writeback_kthread(void *data)
{
	...
	while (!kthread_should_stop()) {
		...
		f2fs_write_meta_caches(sbi, false);
		f2fs_write_node_caches(sbi, false);
	}
	return 0;
}
