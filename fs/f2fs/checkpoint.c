...
void f2fs_write_meta_caches(struct f2fs_sb_info *sbi, bool sync)
{
	...
	if (!sync && get_pages(sbi, F2FS_DIRTY_META) <
	...
	f2fs_sync_meta_caches(sbi, nr_to_write, FS_META_IO);
...
void f2fs_wait_on_all_pages(struct f2fs_sb_info *sbi, int type)
{
	...
	for (;;) {
		...
		if (type == F2FS_DIRTY_META)
			f2fs_sync_meta_caches(sbi, LONG_MAX, FS_META_IO);
...
static int do_checkpoint(struct f2fs_sb_info *sbi, struct cp_control *cpc)
{
	...
	f2fs_sync_meta_caches(sbi, LONG_MAX, FS_CP_META_IO);
