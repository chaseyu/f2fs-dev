...
static bool inode_has_blocks(struct inode *inode, struct f2fs_cached_block *entry)
{
...
bool f2fs_sanity_check_inline_data(struct inode *inode, struct f2fs_cached_block *entry)
{
	if (!f2fs_has_inline_data(inode))
		return false;

	if (inode_has_blocks(inode, entry))
		return false;
...
void f2fs_do_read_inline_data(struct folio *folio, struct f2fs_cached_block *entry)
{
	...
	memcpy_to_folio(folio, 0, inline_data_addr(inode, entry),
		       MAX_INLINE_DATA(inode));
	if (!folio_test_uptodate(folio))
		folio_mark_uptodate(folio);
}

void f2fs_truncate_inline_inode(struct inode *inode, struct f2fs_cached_block *entry,
		u64 from)
{
	void *addr;
	...
	addr = inline_data_addr(inode, entry);
...
int f2fs_read_inline_data(struct inode *inode, struct folio *folio)
{
	...
	entry = f2fs_get_inode_cache(F2FS_I_SB(inode), inode->i_ino);
	if (IS_ERR(entry)) {
		folio_unlock(folio);
	...
	if (folio->index)
		...
		f2fs_do_read_inline_data(folio, entry);
...
int f2fs_convert_inline_inode(struct inode *inode)
{
	...
	set_new_dnode(&dn, inode, entry, entry, 0);
...
int f2fs_write_inline_data(struct inode *inode, struct folio *folio)
{
	...
	f2fs_cache_wait_writeback(entry);
	memcpy_from_folio(inline_data_addr(inode, entry),
					...
					bool use_hash)
{
	...
	entry = f2fs_get_inode_cache(sbi, dir->i_ino);
	if (IS_ERR(entry)) {
		*dentry_block = entry;
		return NULL;
	}

	inline_dentry = inline_data_addr(dir, entry);
	...
	if (de)
		*dentry_block = f2fs_cache_make_dentry_block(entry);
	else
					...
					struct f2fs_cached_block *entry)
{
	...
	inline_dentry = inline_data_addr(inode, entry);
			...
			void *inline_dentry)
{
	...
	set_new_dnode(&dn, dir, entry, NULL, 0);
	err = f2fs_reserve_block(&dn, 0);
	if (err)
		goto out;
	...
	f2fs_truncate_inline_inode(dir, entry, 0);
...
static int f2fs_move_rehashed_dirents(struct inode *dir, struct f2fs_cached_block *entry,
							void *inline_dentry)
{
	void *backup_dentry;
	int err;
...
static int do_convert_inline_dir(struct inode *dir, struct f2fs_cached_block *entry,
							void *inline_dentry)
{
	if (!F2FS_I(dir)->i_dir_level)
		return f2fs_move_inline_dirents(dir, entry, inline_dentry);
	else
		return f2fs_move_rehashed_dirents(dir, entry, inline_dentry);
}
...
int f2fs_try_convert_inline_dir(struct inode *dir, struct dentry *dentry)
{
	...
	if (f2fs_has_enough_room(dir, entry, &fname)) {
		f2fs_put_cache(entry, true);
		goto out_fname;
	}
	...
	err = do_convert_inline_dir(dir, entry, inline_dentry);
	if (!err)
		f2fs_put_cache(entry, true);
out_fname:
			  ...
			  struct inode *inode, nid_t ino, umode_t mode)
{
	...
	if (inode) {
		...
		nentry = f2fs_init_inode_metadata(inode, dir, fname, entry);
		if (IS_ERR(nentry)) {
			err = PTR_ERR(nentry);
			goto fail;
		}
...
void f2fs_delete_inline_entry(struct f2fs_dir_entry *dentry,
		struct f2fs_cached_block *entry, struct inode *dir, struct inode *inode)
{
	struct f2fs_dentry_ptr d;
	void *inline_dentry;
		...
		struct fiemap_extent_info *fieinfo, __u64 start, __u64 len)
{
	...
	if (fieinfo->fi_flags & FIEMAP_FLAG_SYNC) {
		err = f2fs_write_node_cache(entry, true, false, FS_NODE_IO);
		if (err)
			return err;
		entry = f2fs_get_inode_cache(F2FS_I_SB(inode), inode->i_ino);
		if (IS_ERR(entry))
	...
	if (__is_valid_data_blkaddr(ni.blk_addr)) {
		...
		byteaddr += (char *)inline_data_addr(inode, entry) -
						(char *)F2FS_INODE(entry);
	} else {
