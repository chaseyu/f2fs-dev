...
int f2fs_init_security(struct inode *inode, struct inode *dir,
				const struct qstr *qstr, struct f2fs_cached_block *entry)
{
	return security_inode_init_security(inode, dir, qstr,
				f2fs_initxattrs, entry);
}
#endif
...
static int read_inline_xattr(struct inode *inode, struct f2fs_cached_block *entry,
							void *txattr_addr)
{
	...
	if (entry) {
		inline_addr = inline_xattr_addr(inode, entry);
	} else {
...
static int lookup_all_xattrs(struct inode *inode, struct f2fs_cached_block *entry,
				...
				bool *is_inline)
{
	...
	*base_size = XATTR_SIZE(inode) + XATTR_PADDING_SIZE;
	...
	if (inline_size) {
		err = read_inline_xattr(inode, entry, txattr_addr);
		if (err)
			goto out;
...
static int read_all_xattrs(struct inode *inode, struct f2fs_cached_block *entry,
							void **base_addr)
{
...
int f2fs_getxattr(struct inode *inode, int index, const char *name,
		void *buffer, size_t buffer_size, struct f2fs_cached_block *entry)
{
	...
	error = lookup_all_xattrs(inode, entry, index, len, name,
...
static int __f2fs_setxattr(struct inode *inode, int index,
			...
			struct f2fs_cached_block *entry, int flags)
{
	struct f2fs_sb_info *sbi = F2FS_I_SB(inode);
	...
	if (size > MAX_VALUE_LEN(inode))
		return -E2BIG;
retry:
	error = read_all_xattrs(inode, entry, &base_addr);
	if (error)
		return error;
	...
	error = write_all_xattrs(inode, new_hsize, base_addr, entry);
	if (error)
		goto exit;
